...
| ----- | ----- | ---------------------------------------------- | ------ | ------------ | ----------------- |
| TA001 | ✅    | 관리자 권한 모델 설계·부트스트랩 ⭐ - 우선순위 | -      | FA001        | 🟢 OPEN-5(단일)   |
| TA002 | ✅    | 3중 가드(미들웨어/RLS/RPC) 개념·계약 확정      | TA001  | FA001, FA002 | 🟢 OPEN-3(포함)   |

> ✅ **A0 완료(2026-07-04)**: [`adr/0005-admin-permission-model.md`](./adr/0005-admin-permission-model.md)에 판별 방식(`admin_users`)·부트스트랩(0625chopin 지정)·3중 가드 계약·OPEN-1/3/4/5 결정 확정.
...
| Task  | 상태  | 작업                                                     | 의존성 | 관련 기능      | 리스크(OPEN)    |
| ----- | ----- | -------------------------------------------------------- | ------ | -------------- | --------------- |
| TA010 | ✅    | 관리자 공용 타입 정의 (`lib/types/admin*`) ⭐ - 우선순위 | TA001  | 전 기능/데이터 | 🟢 OPEN-4(별도) |
| TA011 | ✅    | `/**` 라우트 골격(빈 페이지) 스캐폴딩                    | TA010  | FA010~FA090    | -               |
...
- **TA020 세부**
  ...
  - [x] shadcn/ui 확충(table/chart는 **shared/ui 승격**, card/badge/dialog 등 shared 재사용) + 관리자 공통 컴포넌트 `components/admin/*`(순수 props): `AdminTable`·`KpiCard`·`TrendChart`·`OpsWidget`·`SystemStatusCard`·`AdminActionDialog`
  - [x] 관리자용 포맷 유틸(집계 수치/비율/상대시각) `lib/format-admin.ts` + 라벨 맵 `lib/labels-admin.ts` — shared `format` 재사용
- **TA021 세부 — 대시보드(FA010~FA013)** (PRD_ADMIN "📊 대시보드 상세")
  - [x] KPI 카드 6종+(총회원/진행중경매/오늘신규/거래완료율/미처리신고/제재중회원), 위젯 클릭 시 해당 목록 이동 링크 — FA010 `1차`
  - [x] 추이 차트(일별 가입·경매·거래) + 기간 토글(7/14일) — FA011 `1차` (카테고리 분포/GMV Mock 보유, 위젯화는 후속)
  - [x] 운영 위젯(마감임박 경매·자동완료 대기 거래·최근 신고·최근 가입) — FA012 `1차`
  - [x] 시스템 상태 카드(pg_cron 잡 close/auto-complete 최근 실행·Storage 개요) — FA013 `2차`
- **TA022 세부 — 회원 관리(FA020~FA025)**
  - [x] 회원 검색/목록: 닉네임·id·지역 검색 + 정지상태 필터 — FA020 `1차`
  - [x] 회원 상세·이력: 레벨·평점·정지/패널티/조치 이력 종합 — FA021 `1차`
  - [x] 상세 내 조치 버튼 UI(정지/해제·패널티 부여·관리자 지정·닉네임 강제변경) — FA022`1차`/FA024`1차`/FA023`2차`/FA025`2차` (인터랙션은 A3)
- **TA023 세부 — 상품/경매 관리(FA030~FA032)**
  - [x] 목록·필터·정렬: 상태(ProductStatus)·판매자·검색, **신고 많은 상품 우선 정렬** — FA030 `1차`
  - [x] 상세 + 조치 버튼(강제 내림/블라인드) — FA031 `1차`, 경매 강제 종료 — FA032 `2차`
...
- **TA031 세부 — 조치 인터랙션(Mock)** 🟢 OPEN-2*(권고 기본안 채택: 1차=정지·패널티·강제내림/취소, 2차=블라인드·경매강제종료·관리자지정·닉네임)
  - [x] 공용 `AdminActionDialog`: **사유 필수 입력** 검증(감사 로그 reason 대비) + 요약 표시 + 완료 토스트 피드백(Mock)
  - [x] 계정 정지/해제 — FA022 `1차` (상세 조건부 정지/해제 버튼)
  - [x] 패널티 수동 부여 — FA024 `1차` (30일 3회 누적 안내, ISSUE-004 정합)
  - [x] 상품 강제 내림 — FA031 `1차`
  - [x] 거래 강제 취소/완료 — FA041 `1차`
  - [x] 관리자 지정·닉네임 강제변경·경매 강제 종료·블라인드(FA023/FA025/FA032) — `2차` 뱃지 구분
...
> 🏁 **마일스톤 AM1 — 관리자 화면 완성**: 전 관리자 페이지가 Mock 데이터로 동작하고 `/sample`에 전시됨. `check-all` + `next build` 통과, Playwright 인터랙션 검증 완료.
>
> ✅ **AM1 달성(2026-07-04)**: Phase A0~A3 완료(TA001~TA033). 전 관리자 화면(대시보드·회원·상품·거래·신고·설정·채팅·평점·통계)이 Mock으로 동작 + `/sample` 전시. 3상태(loading/error/empty)·조치 다이얼로그(사유 필수+토스트+신고 optimistic)·반응형(모바일 아이콘 레일)·다크/라이트 정합. `check-all`+`next build`(동적 [id] ◐ PPR)+Playwright 검증 통과. **다음: A4(DB 설계 & RLS, Supabase MCP) → 마일스톤 AM2.** ⏳ 잔여: shared 레포(table/chart) 커밋, OPEN-6 최종확정.
